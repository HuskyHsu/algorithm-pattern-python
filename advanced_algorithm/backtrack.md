...
回溯法（backtrack）常用於遍曆列錶所有子集，是 DFS 深度搜索一種，一般用於全排列，窮盡所有可能，遍曆的過程實際上是一個決策樹的遍曆過程。時間複雜度一般 O(N!)，它不像動態規劃存在重疊子問題可以優化，回溯算法就是純暴力窮舉，複雜度一般都很高。
...
核心就是從選擇列錶裏做一個選擇，然後一直遞歸往下搜索答案，如果遇到路徑不通，就返回來撤銷這次選擇。
